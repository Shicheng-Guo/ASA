...
input="PDE1A"
mkdir $input
grep -w "$input" ~/hpc/db/hg19/refGene.hg19.V2.bed | awk '{print $1,$2-500000,$3+500000,$5}' OFS="\t" | sort -u | bedtools sort -i > $input.hg19.bed
for i in {1..22} X 
do 
...
sort -u  ROI.assoc.logistic | sort -k12n,12 | head -n 5
sort -u  ROI.assoc.logistic | sort -k12n,12 | tail -n 5
...
plink --bfile ROI.dbsnp --r2 --ld-snp $rs --ld-window-kb 1000 --ld-window 99999 --ld-window-r2 0 --out $rs
