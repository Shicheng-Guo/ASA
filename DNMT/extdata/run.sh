...
input="DNMT"
mkdir $input
grep "DNMT\|TET" ~/hpc/db/hg19/refGene.hg19.V2.bed | awk '{print $1,$2-500000,$3+500000,$5}' OFS="\t" | sort -u | bedtools sort -i > $input.hg19.bed
for i in {1..22} X 
do 
...
sort -u  ROI.assoc.logistic | grep -v 'NA' | sort -k12n,12 | head -n 20
sort -u  ROI.assoc.logistic | grep -v 'NA' |sort -g -k12n,12 | tail -n 20

rs="10:70847318:A:AAAG"
chr=10
...
plink --bfile ROI.dbsnp --r2 --ld-snp $rs --ld-window-kb 500 --ld-window 99999 --ld-window-r2 0 --out $rs
